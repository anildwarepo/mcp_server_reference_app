...
class BackupActor(Actor, BackupActorInterface, Remindable):
    ...
    async def run_backup(self):
        ...
        try:
            ...
            await publish_progress(self.backup_config.user_id, token, 0.0)
            await publish_message(self.backup_config.user_id, f"Running backup for {self.backup_config.file_path}…")
            ...
            await publish_progress(self.backup_config.user_id, token, 1.0)
            # sleep
            await asyncio.sleep(1)
            await publish_message(self.backup_config.user_id, f"Backup completed: src: {os.path.basename(src_path)}  dest: {os.path.basename(dest_path)}")
